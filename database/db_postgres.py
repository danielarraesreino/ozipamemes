        ...
        with self._cur() as cur:
            ...
            cur.execute(
                ...
                (meme_id, conteudo["contexto_oculto"], conteudo["pilula_sabedoria"],
                 conteudo.get("objetivo_meme"), conteudo.get("pilula_alt1"),
                 conteudo.get("pilula_alt2"), conteudo.get("roteiro_tiktok"),
                 conteudo["modelo_claude"], conteudo["prompt_version"],
                 conteudo.get("tokens_input"), conteudo.get("tokens_output")),
            )
            row_id = cur.fetchone()["id"]
        self.conn.commit()
        return row_id
